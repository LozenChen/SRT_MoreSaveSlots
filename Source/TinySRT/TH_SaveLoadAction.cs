...
// 每个 SL Action 能存储一些数据 savedValues, 并且指定了如何 存/读 这些数据
// 而所有 SL Action 又合起来得到 All
public sealed class TH_SaveLoadAction {
    ...
    internal static List<TH_SaveLoadAction> All => SaveSlotsManager.Slot.All;
        ...
        SafeAdd(
            (savedValues, _) => {
                ...
                foreach (EverestModule everestModule in Everest.Modules) {
                    if (everestModule.Metadata?.Name == "CelesteTAS" || everestModule == SpeedrunToolModule.Instance || everestModule == SRT_MoreSaveSlots.Module.SRT_MoreSaveSlotsModule.Instance) {
                        continue;
                    }
