...
using TH_Deep = Celeste.Mod.SRT_MoreSaveSlots.TinySRT.TH_DeepClonerUtils;
...
// why it's named "TH": it's originally part of Tas Helper

// you may find this project quite messy.... coz it's copied from SRT, and modified in TasHelper, and modified again here
public class TH_StateManager {
    internal TH_StateManager() {
    }
...
#pragma warning restore CS8603
    ...
    public static TH_StateManager Instance => SaveSlotsManager.Slot?.StateManager;
    ...
    private static void Load() {
        On.Monocle.Scene.Begin += OnSwitchScene;
    }
    ...
    private static void OnSwitchScene(On.Monocle.Scene.orig_Begin orig, Scene self) {
        orig(self);
        foreach (SaveSlot slot in SaveSlotsManager.SaveSlots) {
            slot.StateManager.ClearStateWhenSwitchScene(self);
        }
    }

    internal void ClearStateWhenSwitchScene(Scene self) {
        if (IsSaved) {
    ...
    // plz use ManagerOfManagers.SaveState
    internal static bool SaveState() {
    ...
    // plz use ManagerOfManagers.LoadState
    internal static bool LoadState() {
        if (Engine.Scene is not Level level) {
            return false;
        }
